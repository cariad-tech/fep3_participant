 ...
 * * FEP3_CLOCK_SERVICE_SLAVE_SYNC_CYCLE_TIME =
 *   FEP3_CLOCK_SERVICE_SLAVE_SYNC_CYCLE_TIME_DEFAULT_VALUE
 * @req_id FEPSDK-2439
 */
TEST_F(NativeClockSyncService, getProperty_syncSlaveCylceTime_100ms)
{
    const std::string timing_master_name_expectation;
    const std::string slave_sync_cycle_time_default_value =
        std::to_string(FEP3_SLAVE_SYNC_CYCLE_TIME_DEFAULT_VALUE);

    // actual test case
    {
        ASSERT_EQ(fep3::base::getPropertyValue<std::string>(
                      *_clock_sync_service_property_node->getChild(FEP3_TIMING_MASTER_PROPERTY)),
                  timing_master_name_expectation);
        ASSERT_EQ(
            fep3::base::getPropertyValue<std::string>(
                *_clock_sync_service_property_node->getChild(FEP3_SLAVE_SYNC_CYCLE_TIME_PROPERTY)),
            slave_sync_cycle_time_default_value);
    }
}

/**
 * @detail Test whether the clock sync service logs the configured timing master name on
 * initialization success.
 */
TEST_F(NativeClockSyncService, init_successful_timingMasterNameLogged)
{
    EXPECT_CALL(*_logger, isDebugEnabled()).WillRepeatedly(Return(true));

    auto main_clock_property_node =
    ...
    fep3::base::setPropertyValue(
        *_clock_sync_service_property_node->getChild(FEP3_TIMING_MASTER_PROPERTY), "timing_master");
    ...
    // actual test case
    {
        ...
        EXPECT_CALL(*_logger,
                    logDebug(fep3::mock::LogStringRegexMatcher("Participant 'timing_master' is")))
            .WillOnce(Return(fep3::Result{}));
        ...
        EXPECT_CALL(*_rpc_requester, sendRequest(_, ContainsRegex(R"("registerSyncSlave")"), _))
            .Times(1)
            .WillOnce(DoAll(
                WithArg<2>(Invoke([&json_reply](fep3::arya::IRPCRequester::IRPCResponse& response) {
                    response.set(json_reply);
        ...
        EXPECT_CALL(*_rpc_requester, sendRequest(_, ContainsRegex(R"("unregisterSyncSlave")"), _))
            .Times(1)
            .WillOnce(DoAll(
                WithArg<2>(Invoke([json_reply](fep3::arya::IRPCRequester::IRPCResponse& response) {
                    response.set(json_reply);
...
/**
 * @detail Test whether the clock sync service returns an error on initialization if no timing
 * master is configured.
 */
TEST_F(NativeClockSyncService, init_failOnEmptyTimingMasterName)
{
    // actual test case
    {
        EXPECT_CALL(*_logger, logError(ContainsRegex("timing master")))
            .Times(1)
...
/**
 * @detail Test whether the clock sync service successfully initializes if no main clock is
 * configured.
 */
TEST_F(NativeClockSyncService, init_successfulOnEmptyMainClockName)
{
    // actual test case
    {
        // set properties to avoid getting an error which we do not test for in this test case
...
/**
 * @detail Test whether the clock sync service returns an error on initialization if an invalid sync
 * cycle time is configured.
 */
TEST_F(NativeClockSyncService, init_failOnZeroSyncCycleTime)
{
    const int64_t invalid_sync_cycle_time = 0;

    // actual test case
    {
        EXPECT_CALL(*_logger, logError(ContainsRegex("sync cycle time")))
            .Times(1)
            .WillOnce(::testing::Return(::fep3::Result{}));
...
/**
 * @detail Test whether the clock sync service successfully initializes if the clock service
 * component does not provide a main clock property node.
 */
TEST_F(NativeClockSyncService, init__successfulOnNoMainClockPropertyNode)
{
    // actual test case
    {
        auto clock_property_node = dynamic_cast<fep3::base::arya::IPropertyWithExtendedAccess*>(
            _clock_service_property_node.get());
