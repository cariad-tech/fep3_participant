...
struct TestLoggingService : public ::testing::Test {
    ...
    std::shared_ptr<fep3::native::ConfigurationService> _configuration_service{
        std::make_shared<fep3::native::ConfigurationService>()};
...
public:
    ...
    void SetUp()
    {
        _logging = std::make_shared<fep3::native::LoggingService>();
...
TEST_F(TestLoggingService, TestLoggingConfiguration)
{
    ...
    ASSERT_NO_THROW(_logging_service_client->setLoggerFilter(
        "", "", static_cast<int>(fep3::LoggerSeverity::fatal)));
    ...
    ASSERT_NO_THROW(_logging_service_client->setLoggerFilter(
        "", "LoggerA.Tester", static_cast<int>(fep3::LoggerSeverity::error)));
    ...
    ASSERT_NO_THROW(_logging_service_client->setLoggerFilter(
        "", "Tester", static_cast<int>(fep3::LoggerSeverity::warning)));
...
TEST_F(TestLoggingService, TestDefaultConfiguration)
{
    ...
    EXPECT_EQ(static_cast<fep3::LoggerSeverity>(default_filter["severity"].asInt()),
...
{
    ASSERT_NO_THROW(_logging_service_client->setLoggerFilter(
        "file", "", static_cast<int>(fep3::LoggerSeverity::error)));

    Json::Value default_filter = _logging_service_client->getLoggerFilter("");
    EXPECT_EQ(default_filter["enable_sinks"].asString(), "file");
    EXPECT_EQ(static_cast<fep3::LoggerSeverity>(default_filter["severity"].asInt()),
              fep3::LoggerSeverity::error);

    // The default stays the same after initialization and is not overwritten by the logging service
    // properties
    _component_registry->initialize();
...
TEST_F(TestLoggingService, TestDefaultConfigurationProperties)
{
    ASSERT_FEP3_NOERROR(fep3::base::setPropertyValue<std::string>(
        *_configuration_service, "logging/default_sinks", "file"));
    ASSERT_FEP3_NOERROR(
        fep3::base::setPropertyValue<int32_t>(*_configuration_service,
                                              "logging/default_severity",
                                              static_cast<int32_t>(fep3::LoggerSeverity::error)));
...
TEST_F(TestLoggingService, TestDefaultConfigurationPropertiesAndRPC)
{
    ...
    ASSERT_FEP3_NOERROR(fep3::base::setPropertyValue<std::string>(
        *_configuration_service, "logging/default_sinks", "console,file"));
    ASSERT_FEP3_NOERROR(
        fep3::base::setPropertyValue<int32_t>(*_configuration_service,
                                              "logging/default_severity",
                                              static_cast<int32_t>(fep3::LoggerSeverity::warning)));
    ...
    // During initialization the default filter doesn't change because RPC has a higher priority
    // than the properties
    _component_registry->initialize();
...
/**
 * Tests that default configuration if set by properties does not delete other filters set before
 * intilization but still changes the root filter
 * @req_id ???
 */
TEST_F(TestLoggingService, TestDefaultConfigurationPropertiesNotDeletingOtherFilters)
{
    ...
    ASSERT_NO_THROW(_logging_service_client->setLoggerFilter(
        "rpc", "participant", static_cast<int>(fep3::LoggerSeverity::off)));
    ...
    EXPECT_EQ(static_cast<fep3::LoggerSeverity>(participant_filter["severity"].asInt()),
              fep3::LoggerSeverity::off);
...
struct TestLoggingServiceEnvVariable : public TestLoggingService {
    void SetUp()
    {
        a_util::process::setEnvVar("FEP3_DEFAULT_LOGGING_SEVERITY", "5");
        TestLoggingService::SetUp();
    }

    void TearDown()
    {
        a_util::process::setEnvVar("FEP3_DEFAULT_LOGGING_SEVERITY", "");
        TestLoggingService::TearDown();
    }
};

TEST_F(TestLoggingServiceEnvVariable, TestSetSeverityOverEnvironmentVariable)
{
    Json::Value default_filter = _logging_service_client->getLoggerFilter("");

    EXPECT_EQ(default_filter["enable_sinks"].asString(), "console,rpc");
    EXPECT_EQ(static_cast<fep3::LoggerSeverity>(default_filter["severity"].asInt()),
              fep3::LoggerSeverity::debug);
}
