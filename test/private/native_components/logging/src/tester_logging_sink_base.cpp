...
#include"fep3/native_components/logging/sinks/logging_formater_json.hpp"
...
namespace fep3
{
    namespace arya
    {
        ...
        {
            return ((lhs._logger_name == rhs._logger_name) &&
                (lhs._message == rhs._message) &&
                (lhs._participant_name == rhs._participant_name) &&
                (lhs._severity == rhs._severity) &&
                (lhs._timestamp == rhs._timestamp));
        }
    }
}

struct TestLoggingSinkFileBase : public TestLoggingSinkFolderSetup
{
protected:
    TestLoggingSinkFileBase()
    {
        std::unique_ptr<MockLoggingFormater> logging_formatter = std::make_unique<StrictMock<MockLoggingFormater>>();
        _mock_logging_formatter = logging_formatter.get();
        _logging_sink_file_base = std::make_unique<fep3::LoggingSinkFileBase>(std::move(logging_formatter));
    }

    void SetUp()
    {
        TestLoggingSinkFolderSetup::SetUp();
        std::ofstream log_file(_test_log_file_string);
    }

    std::unique_ptr<fep3::LoggingSinkFileBase> _logging_sink_file_base;
...
{
    fep3::arya::LogMessage logMessage = { "30.08.2021", fep3::arya::LoggerSeverity::debug, "participant", "logger", _log_message };

    EXPECT_CALL(*_mock_logging_formatter, IsStreamAppendable(_)).WillOnce(Return(true));
    EXPECT_CALL(*_mock_logging_formatter, GetStreamBegin()).WillOnce(Return(_init_string));
    EXPECT_CALL(*_mock_logging_formatter, formatLogMessage(Eq(logMessage))).WillOnce(Return(_log_message));
    EXPECT_CALL(*_mock_logging_formatter, StreamEnd(_)).WillOnce(Invoke([](std::ostream & os) { os << "endString"; }));
    ...
    ASSERT_TRUE(readFileContent());
    ASSERT_NO_FATAL_FAILURE(findStringInFile(_init_string + _log_message)) << "Logged string not found in log file";
...
TEST_F(TestLoggingSinkFileBase, TestFileNotAppendable)
{
    ...
    ASSERT_FALSE(_logging_sink_file_base->setProperty("file_path", _test_log_file_string, "string"));
