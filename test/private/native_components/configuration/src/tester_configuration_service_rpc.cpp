...
class TestClient
    : public fep3::rpc::RPCServiceClient<::test::rpc_stubs::TestConfigurationServiceProxy,
                                         fep3::rpc::IRPCConfigurationDef> {
private:
    using base_type = fep3::rpc::RPCServiceClient<::test::rpc_stubs::TestConfigurationServiceProxy,
                                                  fep3::rpc::IRPCConfigurationDef>;
...
TEST_F(NativeConfigurationServiceRPC, getProperty)
{
...

    {
        ...
        EXPECT_EQ(client.getProperty("/Clock/Clocks/Clock1/CycleTime")["type"],
                  base::PropertyType<int32_t>::getTypeName().c_str());
        ...
        EXPECT_EQ(client.getProperty("Clock/Clocks/Clock1")["type"],
                  base::PropertyType<std::string>::getTypeName().c_str());

        EXPECT_EQ(client.getProperty("Clock/Clocks")["value"], "2");
        EXPECT_EQ(client.getProperty("Clock/Clocks")["type"],
                  base::PropertyType<int32_t>::getTypeName().c_str());

        EXPECT_EQ(client.getProperty("Clock/Clocks/Clock2")["value"], "");
        EXPECT_EQ(client.getProperty("Clock/Clocks/Clock2")["type"],
                  base::PropertyType<base::NodePropertyType>::getTypeName().c_str());

        EXPECT_EQ(client.getProperty("Clock")["value"], "");
        EXPECT_EQ(client.getProperty("Clock")["type"],
                  base::PropertyType<base::NodePropertyType>::getTypeName().c_str());
    }
...
TEST_F(NativeConfigurationServiceRPC, existProperty)
{
    ...
    // the root node must exist because otherwise we have no possibility to find out the values of
    // the first nodes
...
TEST_F(NativeConfigurationServiceRPC, getProperties)
{
    ...
    EXPECT_EQ(split(client.getProperties("Clock/Clocks"), ","),
              std::vector<std::string>({"Clock1", "Clock2"}));

    EXPECT_EQ(split(client.getProperties("Clock"), ","), std::vector<std::string>({"Clocks"}));

    EXPECT_EQ(split(client.getProperties(""), ","),
              std::vector<std::string>({"service_bus", "Clock", "second"}));
...
TEST_F(NativeConfigurationServiceRPC, getAllProperties)
{
    ...
    EXPECT_EQ(split(client.getAllProperties("Clock/Clocks"), ","),
              std::vector<std::string>({"Clock/Clocks/Clock1",
                                        "Clock/Clocks/Clock1/CycleTime",
                                        "Clock/Clocks/Clock2",
                                        "Clock/Clocks/Clock2/CycleTime"}));

    EXPECT_EQ(split(client.getAllProperties("Clock"), ","),
              std::vector<std::string>({"Clock/Clocks",
                                        "Clock/Clocks/Clock1",
                                        "Clock/Clocks/Clock1/CycleTime",
                                        "Clock/Clocks/Clock2",
                                        "Clock/Clocks/Clock2/CycleTime"}));
...
TEST_F(NativeConfigurationServiceRPC, setProperty_thatIsExisting)
{
    ...
    {
        EXPECT_EQ(properties_clock->getChild("Clocks")
                      ->getChild("Clock1")
                      ->getChild("CycleTime")
                      ->getValue(),
                  "1");
...
TEST_F(NativeConfigurationServiceRPC, setProperty_differentType)
{
    ...
    {
        EXPECT_EQ(client.setProperty("Clock/Clocks/Clock1/CycleTime", "double", "2.0"),
                  fep3::ResultType_ERR_INVALID_TYPE::getCode());
    }
}
...
TEST_F(NativeConfigurationServiceRPC, setProperty_thatIsNotExisting)
{
    ...
    EXPECT_EQ(client.setProperty("Clock/Clocks/Clock1/CycleTimeNew", "", "2"),
              fep3::ResultType_ERR_NOT_FOUND::getCode());
