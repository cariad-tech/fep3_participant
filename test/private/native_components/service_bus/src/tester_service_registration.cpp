...
class TestService
    : public fep3::rpc::RPCService<::test::rpc_stubs::TestInterfaceServer, ITestInterface> {
    int _value = 0;
...
class TestClient
    : public fep3::rpc::RPCServiceClient<::test::rpc_stubs::TestInterfaceClient, ITestInterface> {
public:
    TestClient(const char* service_name, const std::shared_ptr<fep3::IRPCRequester>& rpc)
        : fep3::rpc::RPCServiceClient<::test::rpc_stubs::TestInterfaceClient, ITestInterface>(
              service_name, rpc)
    {
    }
};
...
TEST(ServiceBusServer, testRegistrationOfServices)
{
    ...
    ASSERT_TRUE(bus.createSystemAccess("sysname", "", true));

    auto sys_access = bus.getSystemAccess("sysname");
    ASSERT_TRUE(sys_access);

    // now create the server
    ASSERT_TRUE(sys_access->createServer("name_of_server", test_server_url));
    ...
    ASSERT_TRUE(server->registerService("test_service", test_service));

    // register twice is not possible
    ASSERT_FALSE(server->registerService("test_service", test_service));

    // impl test
    fep3::rpc::RPCClient<ITestInterface> my_interface_client;

    // test the client server connections
    TestClient client(ITestInterface::getRPCDefaultName(), bus.getRequester(test_server_url, true));

    ASSERT_NO_THROW(ASSERT_EQ(TestService::SetRunlevel_call_count, 0);
                    auto val = client.SetRunlevel(1234);
                    ASSERT_EQ(TestService::SetRunlevel_call_count, 1););

    ASSERT_NO_THROW(ASSERT_EQ(TestService::GetRunlevel_call_count, 0);
                    auto run_level = client.GetRunlevel();
                    ASSERT_EQ(TestService::GetRunlevel_call_count, 1);
                    ASSERT_EQ(run_level, 1234););

    ASSERT_NO_THROW(ASSERT_EQ(TestService::GetObjects_call_count, 0);
                    auto objects = client.GetObjects();
                    ASSERT_EQ(TestService::GetObjects_call_count, 1);
                    ASSERT_EQ(objects, "bla, blubb, bla"););

    ASSERT_NO_THROW(ASSERT_EQ(TestService::GetRPCIIDForObjects_call_count, 0);
    ...
    ASSERT_NO_THROW(auto object_IID = client.GetRPCIIDForObject("blubb");
                    ASSERT_EQ(TestService::GetRPCIIDForObjects_call_count, 2);
                    ASSERT_EQ(object_IID, "bla"););

    ASSERT_NO_THROW(auto object_IID = client.GetRPCIIDForObject("test");
                    ASSERT_EQ(TestService::GetRPCIIDForObjects_call_count, 3);
                    ASSERT_EQ(object_IID, ""););

    // unregister the service
    ASSERT_TRUE(server->unregisterService("test_service"));

    ASSERT_ANY_THROW(auto object_IID = client.GetRPCIIDForObject("test"););
...
TEST(ServiceBusServer, testRegistrationOfMultipleServices)
{
    ...
    // test the client server connections
    TestClient client_1("test_service_1", bus.getRequester(test_server_url, true));

    TestClient client_2("test_service_2", bus.getRequester(test_server_url, true));

    ASSERT_ANY_THROW(client_1.SetRunlevel(10));
    ASSERT_ANY_THROW(client_2.SetRunlevel(20));

    // register the first service
    ASSERT_TRUE(server->registerService("test_service_1", test_service));

    ASSERT_NO_THROW(auto val = client_1.SetRunlevel(1234);
                    ASSERT_EQ(TestService::SetRunlevel_call_count, 1););

    ASSERT_ANY_THROW(client_2.SetRunlevel(200));

    // register the second service
    ASSERT_TRUE(server->registerService("test_service_2", test_service_simple));

    ASSERT_NO_THROW(auto val = client_1.SetRunlevel(1234);
                    ASSERT_EQ(TestService::SetRunlevel_call_count, 2););

    ASSERT_NO_THROW(ASSERT_EQ(test_service_simple->SetRunlevel_call_count, 0);
    ...
    ASSERT_TRUE(server->unregisterService("test_service_2"));
    ASSERT_TRUE(server->unregisterService("test_service_1"));
}
