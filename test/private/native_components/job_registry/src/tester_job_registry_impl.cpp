...
struct JobRegistryImplTest : public testing::Test {
    JobRegistryImplTest() : _job_registry_impl(_job_registry_configuration)
    {
    }
...
{
    auto my_job =
        std::unique_ptr<fep3::core::Job>(new fep3::core::Job("name", duration_cast<Duration>(1us)));
    fep3::arya::JobConfiguration my_config(duration_cast<Duration>(1us));

    ASSERT_FEP3_RESULT(_job_registry_impl.addJob("name", std::move(my_job), my_config),
                       fep3::ERR_NOERROR);
...
TEST_F(JobRegistryImplTest, AddJobTwice)
{
    ...
    auto my_job = std::unique_ptr<fep3::core::Job>(
        new fep3::core::Job("job_name", duration_cast<Duration>(1us)));
    auto my_job2 = std::unique_ptr<fep3::core::Job>(
        new fep3::core::Job("job_name", duration_cast<Duration>(1us)));

    ASSERT_FEP3_RESULT(_job_registry_impl.addJob("job_name", std::move(my_job), my_config),
                       fep3::ERR_NOERROR);
    ASSERT_FEP3_RESULT(_job_registry_impl.addJob("job_name", std::move(my_job2), my_config),
...
TEST_F(JobRegistryImplTest, GetJobsAndCheckJobInfo)
{
    ...
    {
        auto my_job = std::unique_ptr<fep3::core::Job>(
            new fep3::core::Job("some_other_jobname", duration_cast<Duration>(1us)));

        auto cycle_sim_time = 1us;
        auto delay_sim_time = 1us;
        Optional<Duration> max_runtime_real_time = 2us;
        auto runtime_violation_strategy =
            fep3::JobConfiguration::TimeViolationStrategy::ignore_runtime_violation;

        fep3::arya::JobConfiguration my_config(duration_cast<Duration>(cycle_sim_time),
