...
TEST_F(DiscreteClockTest, testClockProvidesDiscreteTimeSteps)
{
    ...
    std::vector<Timestamp>expected_timestamps{
        Timestamp{ step_size * 0 } };  // First timestamp originating from a reset event once the clock is started

    for (size_t i = 1; i < clock_cycles; i++)
    {
        expected_timestamps.emplace_back(Timestamp( i * step_size));  // Timestamps originating from time update events
    }

    local_system_sim_clock.updateConfiguration(Duration{ step_size }, time_factor);
...
TEST_F(DiscreteClockTest, testClockTimeEventFrequency)
{
    ...
    const Duration step_size{ duration_cast<Duration>(50ms) },
    ...
    local_system_sim_clock.updateConfiguration(step_size, time_factor);
...
TEST_F(DiscreteClockTest, testClockTimeEventFrequencyBigTimeFactor)
{
    ...
    // we have to use 10 ms as step size due to performance limitations of the discrete clock
    const Duration step_size{ duration_cast<Duration>(50ms) },
...
TEST_F(DiscreteClockTest, testClockTimeEventAFAP)
{
    ...
    // Step size value does not matter in case of this test due to time factor being 0.
    // This test succeeds if event durations are below 2 ms as this durations can not be
    // reached when using a time factor != 0.
    const Duration step_size{ duration_cast<Duration>(20ms) },
