...
const std::string relative_path_components_file_path_source = std::string(TEST_BUILD_DIR) + "/files/test_rel_path.fep_components";
...
/**
 * Creation of a component registry with built in components is not possible, a fep_components file should be always defined.
 * @req_id FEPSDK-3218
 */
TEST(ComponentRegistryFactoryTester, testComponentRegistryCreationDefault)
{
    ...
    ("searching for default FEP Component Configuration File")))
        .WillOnce(::testing::Return(fep3::Result{}));
    EXPECT_CALL(logger_mock, isErrorEnabled()).WillRepeatedly(::testing::Return(true));
    EXPECT_CALL(logger_mock, logError(_)).WillOnce(::testing::Return(fep3::Result{}));


    std::shared_ptr<fep3::arya::ComponentRegistry> registry;
    EXPECT_THROW
    (fep3::arya::ComponentRegistryFactory::createRegistry(&logger_mock), std::runtime_error);
}
...
TEST(ComponentRegistryFactoryTester, testComponentRegistryCreationByFile)
{
    ...
    ASSERT_NO_THROW(testLoadingAllComponents()) << "Component creation by file test failed";
}
 ...
 * @req_id FEPSDK-2417
 */
TEST(ComponentRegistryFactoryTester, testComponentRegistryCreationByFileWithRelativeComponentPaths)
{
    // make sure the environment variable is not set (e. g. due to previous test runs)
    unsetEnvironmentVariable("FEP3_PARTICIPANT_COMPONENTS_FILE_PATH");

    ASSERT_TRUE(test::helper::copyFile(relative_path_components_file_path_source, std::string(TEST_BUILD_DIR) + "/fep3_participant.fep_components"));

    ASSERT_NO_THROW(testLoadingAllComponents()) << "Component creation by file containing relative paths test failed";
}
