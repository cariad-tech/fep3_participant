...
#define EXPECT_DESTRUCTION(mock_object_to_be_checked)                                              \
    EXPECT_CALL((mock_object_to_be_checked), die())                                                \
        .WillOnce(::testing::Invoke((mock_object_to_be_checked).getDestructionChecker().get(),     \
                                    &test::helper::detail::DestructionChecker::call));             \
...
public:
    Dieable()
        : _mock_destruction_checker(
              std::make_shared<::testing::StrictMock<detail::DestructionChecker>>())
    {
