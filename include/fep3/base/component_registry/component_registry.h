 ...
 *
 * @note Objects of this class must be held in an std::shared_ptr, because this class is derived
 * from std::enable_shared_from_this
 */
class ComponentRegistry : public arya::IComponents,
...
public:
     ...
     * @pre @p component_type must implement access to the interface id (see @ref
     * FEP3_COMPONENT_IID)
     ...
     * @param[in] version_info Version information of the component to be registered
     ...
     * The given component must implement an access to an interface identifier @p fep_iid (see @ref
     * FEP3_COMPONENT_IID)
     * @remark The component registry takes shared ownership of the @p component if and only if
     * registration is successful. Only one component of the interface @p fep_iid can be registered.
     *
    ...
    /**
     * @brief Returns the component version of a loaded registered iid.
     *
     *
     * @param[in] fep_iid the component interface identifier of the component
     * @return std::pair<fep3::Result, ComponentVersionInfo> if component iid is registered the
     * second member of the pair is the registered component's version info. If the component iid is
     * not registered the first member of the pair will be fep3::ERR_INVALID_ARG and the second
     * member is undefined.
     */
    std::pair<fep3::Result, ComponentVersionInfo> getComponentVersion(
        const std::string& fep_iid) const;

    /**
     * @brief Returns the registered components iids.
     *
     *
     * @return std::vector<std::string> the iids of the registered components.
     */
    std::vector<std::string> getComponentIIDs() const;
...
private:
    ...
    /// the version information of the registered components
