                              ...
                              size_t queue_capacity = 1)
{
    ...
    reader = res ? data_registry.getReader(name, queue_capacity) : nullptr;
    return res;
}
...
{
    const auto res = data_registry.registerDataOut(name, stream_type);
    writer = res ? data_registry.getWriter(name, queue_capacity) : nullptr;
    return res;
}
 ...
 * @retval fep3::ERR_INVALID_ARG  Error target not found in mapping structure
 * @retval fep3::ERR_FAILED Removal of mapping failed
 */
inline fep3::Result removeDataIn(fep3::arya::IDataRegistry& data_registry, const std::string& name)
{
 ...
 */
inline fep3::Result removeDataOut(fep3::arya::IDataRegistry& data_registry, const std::string& name)
{
