 ...
 * The capacity of the DataItemQueue is fixed. If samples are pushed into the queue while the
 * queue's capacity is reached old items are dropped.
 *
 * |Nr. sample|Queue|
 * |1.        |front|
 * |2.        |2.   |
...
class FEP3_PARTICIPANT_EXPORT DataItemQueue
    : public arya::detail::DataItemQueueBase<const fep3::arya::IDataSample,
...
public:
     ...
     */
    DataItemQueue(const DataItemQueue&) = delete;

    /**
     * @brief Deleted Move CTOR
     */
    DataItemQueue(DataItemQueue&&) = delete;
     ...
     */
    DataItemQueue& operator=(const DataItemQueue&) = delete;
     ...
     * @return DataItemQueue&
     */
    DataItemQueue& operator=(DataItemQueue&&) = delete;
    ...
    void pushSample(const data_read_ptr<SAMPLE_TYPE>& sample,
                    fep3::arya::Timestamp time_of_receiving) override;

    ///@copydoc fep3::base::arya::detail::DataItemQueueBase::pushType
    void pushType(const data_read_ptr<STREAM_TYPE>& type,
                  fep3::arya::Timestamp time_of_receiving) override;
    ...
    bool popFront(
        typename DataItemQueueBase<SAMPLE_TYPE, STREAM_TYPE>::IDataItemReceiver& receiver) override;
    ...
    std::tuple<data_read_ptr<SAMPLE_TYPE>, data_read_ptr<STREAM_TYPE>> read(
     ...
     */
    void reverseIteration(const std::function<bool(const data_read_ptr<SAMPLE_TYPE>& sample,
                                                   const data_read_ptr<STREAM_TYPE>& stream_type,
                                                   fep3::arya::Timestamp)>& callback) const;
     ...
     * @param callback callback function will be called for each item in the queue
     */
    void iteration(const std::function<bool(const data_read_ptr<SAMPLE_TYPE>& sample,
                                            const data_read_ptr<STREAM_TYPE>& stream_type,
                                            fep3::arya::Timestamp)>& callback) const;
