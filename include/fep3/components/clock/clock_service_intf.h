...
/**
 * @brief Minimum value of the built-in 'discrete simulation time clock' step size property in
 * nanoseconds.
 */
#define FEP3_CLOCK_SIM_TIME_STEP_SIZE_MIN_VALUE 1
/**
 * @brief Maximum value of the built-in 'discrete simulation time clock' step size property in
 * nanoseconds. The value is chosen to allow 1000 simulation steps before reaching the hard limit of
 * int64_t.
 */
#define FEP3_CLOCK_SIM_TIME_STEP_SIZE_MAX_VALUE fep3::Duration::max().count() / 1000
/**
 * @brief Default value of the built-in 'discrete simulation time clock' step size property in
 * nanoseconds.
 */
#define FEP3_CLOCK_SIM_TIME_STEP_SIZE_DEFAULT_VALUE 100000000

/**
 * @brief Factor at which discrete time steps of the built-in discrete simulation time clock pass
 * compared to the system time. A time factor of 2 means the discrete time step passes twice as fast
 * compared to the system time. A time factor of 0,0 means no delay exists between discrete time
 * steps.
 */
 ...
 * steps.
 */
#define FEP3_CLOCK_SERVICE_CLOCK_SIM_TIME_TIME_FACTOR                                              \
    FEP3_CLOCK_SERVICE_CONFIG "/" FEP3_CLOCK_SIM_TIME_TIME_FACTOR_PROPERTY
/**
 * @brief Value to configure the built-in 'discrete simulation time clock' time factor property to
 * run in 'As Fast As Possible' mode.
 */
...
/**
 * @brief Minimum value of the built-in 'discrete simulation time clock' wall clock step size in
 * nanoseconds. The wall clock step size takes into account the @ref
 * FEP3_CLOCK_SERVICE_CLOCK_SIM_TIME_STEP_SIZE and the
 * @ref FEP3_CLOCK_SERVICE_CLOCK_SIM_TIME_TIME_FACTOR. The configured step size may not be lower
 * than this value after applying the time factor.
 */
 ...
 * FEP3_CLOCK_SERVICE_CLOCK_SIM_TIME_STEP_SIZE and the
 * @ref FEP3_CLOCK_SERVICE_CLOCK_SIM_TIME_TIME_FACTOR. The configured step size may not be lower
 * than this value after applying the time factor. The value is chosen to allow 1000 simulation
 * steps before reaching the hard limit of int64_t.
 */
#define FEP3_CLOCK_WALL_CLOCK_STEP_SIZE_MAX_VALUE fep3::Duration::max().count() / 1000
/**
 * @brief Name of the clock service built-in clock to retrieve the current system time (continuous
 * clock).
 ...
 * The discrete clock may be configured using @ref FEP3_CLOCK_SIM_TIME_STEP_SIZE_PROPERTY and @ref
...
/**
 * @brief Timeout for sending time update events like timeUpdating, timeReset, etc. to timing
 * slaves. The timeout is applied per slave and update function.
 */
 ...
 * slaves. The timeout is applied per slave and update function.
 */
#define FEP3_CLOCK_SERVICE_TIME_UPDATE_TIMEOUT                                                     \
    FEP3_CLOCK_SERVICE_CONFIG "/" FEP3_TIME_UPDATE_TIMEOUT_PROPERTY
/**
...
/**
 * @brief Minimum value of @ref FEP3_CLOCK_SERVICE_TIME_UPDATE_TIMEOUT in nanoseconds.
 * If @ref FEP3_CLOCK_SERVICE_TIME_UPDATE_TIMEOUT is configured to be lower than this minimum value,
 * a warning will be logged and this minimum value will be used instead.
 */
#define FEP3_TIME_UPDATE_TIMEOUT_MIN_VALUE 1000000000
 ...
 *
 * The clock service may be used to register custom clocks and set the active main clock for the
 * participant.
 */
class IClockService : public arya::IClockRegistry {
public:
    /**
     * @brief Definition of the clock service component IID.
     */
...
public:
     ...
     */
    virtual arya::Optional<arya::IClock::ClockType> getType(
        const std::string& clock_name) const = 0;

    /**
     * @brief Get the name of the current main clock.
     *
     * @return Before the component is tensed the value of main clock property node, otherwise the
     * name of the current main clock
     */
     ...
     */
    virtual fep3::Result registerEventSink(
        const std::weak_ptr<arya::IClock::IEventSink>& clock_event_sink) = 0;
     ...
     * @retval ERR_POINTER        The @p clock_event_sink weak_ptr is expired.
     */
    virtual fep3::Result unregisterEventSink(
        const std::weak_ptr<arya::IClock::IEventSink>& clock_event_sink) = 0;
};
 ...
 * participant.
 */
class IClockService : virtual public arya::IClockService, public experimental::IClockRegistry {
public:
    /**
     * @brief Definition of the clock service component IID.
     */
    FEP_COMPONENT_IID("clock_service.experimental.fep3.iid")
    using arya::IClockService::registerClock;
    using arya::IClockService::registerEventSink;
    using arya::IClockService::unregisterEventSink;
    using experimental::IClockRegistry::registerClock;
    /**
     * @brief Register an event sink to receive time events.
     *
     * @param[in] clock_event_sink The event sink to register
     ...
     */
    virtual fep3::Result registerEventSink(
        const std::weak_ptr<experimental::IClock::IEventSink>& clock_event_sink) = 0;
    /**
     * @brief Unregister the event sink.
     *
     * @param[in] clock_event_sink The event sink to unregister
