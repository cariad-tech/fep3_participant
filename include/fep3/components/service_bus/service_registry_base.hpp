...
public:
    ...
    virtual std::shared_ptr<fep3::arya::IRPCServer::IRPCService> getServiceByName(
        const std::string& service_name) const = 0;
    virtual std::string getParticipantName() const = 0;
    virtual std::string getSystemName() const = 0;
};

class ServiceRegistryRPCService
    : public rpc::arya::RPCService<fep3::rpc_stubs::RPCParticipantInfoServiceStub,
...
public:
    ...
    std::string getRPCServiceInterfaceDefinition(const std::string& rpc_service_name,
    ...
    std::string getFEPVersion() override
    {
...
public:
     ...
     *
     * @param name name of the participant server to register (this will be the
     * IServiceBus::IParticipantServer)
     * @param system_name the participants system it belongs to (might change while runtime!!)
     */
    ServiceRegistryBase(const std::string& name, const std::string& system_name)
     ...
     * @remark Unfortunately, this MUST be called separately since virtual functional calls are not
     * possible by a CTOR We could also fake here, that the first registerService call will call
     * also for IRPCParticipantInfoDef!
     * @return returns the error values of arya::IServiceBus::IParticipantServer::registerService
     */
    virtual fep3::Result initialize()
    {
...
public:
    // rpc service ... needs to override it
    // virtual std::vector<std::string> getRegisteredServiceNames() const = 0;
    // virtual std::shared_ptr<arya::IRPCServer::IRPCService> getServiceByName(
    //     const std::string& service_name) const = 0;

public:
    // internal rpc server... needs to override it
