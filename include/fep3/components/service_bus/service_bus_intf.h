...
namespace arya
{
    ...
    public:
        ...
        public:
             ...
             *
             * @param[in] server_name name of the server (within FEP 3 this is the participant name to appear)
         ...
         * @param[in] system_discovery_url the network address url to communicate with if discovery is used
        ...
        * @return the server if it does exist. will return default system access, if system_name is empty.
        * @retval empty std::shared_ptr<ISystemAccess>() the server if it does not exist
        */
        virtual std::shared_ptr<ISystemAccess> getSystemAccess(const std::string& system_name = std::string()) const = 0;
