 ...
 * Use this to set the mapping configuration with a file path from inside the data registry
 * configuration node.
 */
 ...
 */
#define FEP3_DATA_REGISTRY_MAPPING_CONFIGURATION_FILE_PATH                                         \
    FEP3_DATA_REGISTRY_CONFIG "/" FEP3_MAPPING_CONFIGURATION_FILE_PATH_PROPERTY

/**
 * @brief The semicolon-separated mapping ddl file paths property name
 * Use this to set the set of mapping ddl files by file path from inside the data registry
 * configuration node.
 */
#define FEP3_MAPPING_DDL_FILE_PATHS_PROPERTY "mapping_ddl_file_paths"

/**
 * @brief The semicolon-separated mapping ddl file paths property name
 * Use this to set the set of mapping ddl files by file path.
 */
#define FEP3_DATA_REGISTRY_MAPPING_DDL_FILE_PATHS                                                  \
    FEP3_DATA_REGISTRY_CONFIG "/" FEP3_MAPPING_DDL_FILE_PATHS_PROPERTY

/**
 * @brief The input signal renaming configuration property name.
 * Use this to set the input signal renaming configuration with a single string from inside the data
 ...
 */
#define FEP3_DATA_REGISTRY_SIGNAL_RENAMING_INPUT_CONFIGURATION                                     \
    FEP3_DATA_REGISTRY_CONFIG "/" FEP3_SIGNAL_RENAMING_INPUT_CONFIGURATION_PROPERTY

/**
 * @brief The output signal renaming configuration property name.
 * Use this to set the output signal renaming configuration with a single string from inside the
 * data registry configuration node.
 */
...
public:
    /// DataReceiver class provides an callback entry for the @ref
    /// fep3::arya::IDataRegistry::registerDataReceiveListener function to receive data as a
    /// synchronous call (data triggered)
    ...
    public:
         ...
         * @remark If "data triggered" reception is currently running (see method @ref
         * fep3::IDataRegistry::registerDataReceiveListener) the reader queue is always empty
         * because incoming data will immediately be passed to the "data triggered" receivers.
         *
    ...
    public:
         ...
         * @return fep3::Result ERR_NOERROR if succeeded, error code otherwise (@see
         * fep3::IDataRegistry::IDataWriter::write(const IDataSample&))
         */
         ...
         * @retval ERR_NOT_INITIALISED Data registry has not been initialized
         * @retval ERR_UNEXPECTED      An unexpected error occurred.
         * @retval ERR_NOT_CONNECTED   The data writer is not connected to a simulation bus
         * resource.
    ...
    /**
     * @brief Registers an incoming date with the given @p name to the simulation bus when the
     * participant is done initializing. This implementation will *NOT* immediately register it to
     * the simulation bus synchronously within this call.
     *
     * @param[in] name The name of the incoming data (must be unique and contain alphanumeric
     * characters or underscore only)
    ...
    /**
     * @brief Registers an outgoing date with the given @p name to the simulation bus when the
     * participant is done initializing. This implementation will *NOT* immediately register it to
     * the simulation bus synchronously within this call.
     *
     * @param[in] name The name of the outgoing data (must be unique and contain alphanumeric
     * characters or underscore only)
     * @param[in] type The stream type of this data (see @ref fep3::arya::IStreamType)
     ...
     * @see registerDataIn
     */
    virtual fep3::Result registerDataReceiveListener(
        const std::string& name, const std::shared_ptr<IDataReceiver>& listener) = 0;
     ...
     * @retval ERR_NOT_FOUND No incoming data with this name found
     */
    virtual fep3::Result unregisterDataReceiveListener(
        const std::string& name, const std::shared_ptr<IDataReceiver>& listener) = 0;

    /**
     * @brief Get a reader for the incoming data with the given @p name. Queue capacity is 1, so
     * only the last item will be read.
     * @note Since incoming data and their readers get registered at the simulation bus only after
     * the initialization of the participant is done this method has to be called during
     * initialization or before.
     *
     * @param[in] name Name of the incoming data
     * @exception std::runtime_error Bad memory allocation
     * @return std::unique_ptr<IDataReader> The return pointer is only a nullptr if the signal is
     * not registered with DataRegistry::addDataIn beforehand and the reader methods will return
     * ERR_NOT_INITIALISED if the module is not at least in state FS_READY or the incoming data has
     * been unregistered.
     */
     ...
     *
     * @param[in] name Name of the incoming data
     * @param[in] queue_capacity The maximum number of items that the reader queue can hold at a
     * time
     * @exception std::runtime_error Bad memory allocation
     * @return std::unique_ptr<IDataReader> The return pointer is only a nullptr if the signal is
     * not registered with DataRegistry::addDataIn beforehand and the reader methods will return
    ...
    /**
     * @brief Get a writer for the outgoing data with the given @p name. Queue capacity is 0, so
     * data will be written immediately.
     * @note Since outgoing data and their writers get registered at the simulation bus only after
     * the initialization of the participant is done this method has to be called during
     * initialization or before.
     *
     * @param[in] name Name of the outgoing data
     * @exception std::runtime_error Bad memory allocation
     * @return std::unique_ptr<IDataWriter> The return pointer is always valid but the writer
     * methods will return ERR_NOT_INITIALISED if the module is not at least in state FS_READY or
     * the outgoing data has been unregistered.
     ...
     * If queue_capacity is not 0 the writer will request a specific amount of sample buffer from
     * the simulation bus.
     * @note Since outgoing data and their writers get registered at the simulation bus only after
     * the initialization of the participant is done this method has to be called during
     * initialization or before.
     *
     * @param[in] name Name of the outgoing data
     * @param[in] queue_capacity The maximum number of items that the transmit queue can hold at a
     * time.
     * @exception std::runtime_error Bad memory allocation
     * @return std::unique_ptr<IDataWriter> The return pointer is always valid but the writer
     * methods will return ERR_NOT_INITIALISED if the module is not at least in state FS_READY or
 ...
 * @param[in] name The name of the incoming data (must be unique and contain alphanumeric characters
 * or underscore only)
 ...
 * @return std::unique_ptr<IDataRegistry::IDataReader> The return pointer is always valid but the
 * reader methods will return ERR_NOT_INITIALISED if the module is not at least in state FS_READY or
 * the incoming data has been unregistered.
 */
[[deprecated(
    "fep3::addDataIn is deprecated. Please use fep3::base::addDataIn instead.")]] inline std::
    unique_ptr<IDataRegistry::IDataReader>
    addDataIn(arya::IDataRegistry& data_registry,
              const std::string& name,
              const arya::IStreamType& stream_type,
              size_t queue_capacity = 1)
{
    const auto res = data_registry.registerDataIn(name, stream_type);
    return res ? data_registry.getReader(name, queue_capacity) : nullptr;
}
 ...
 * @param[in] name The name of the outgoing data (must be unique and contain alphanumeric characters
 * or underscore only)
 * @param[in] stream_type The stream type of this data (see @ref fep3::arya::IStreamType)
 * @param[in] queue_capacity The maximum number of items that the transmit queue can hold at a time.
 * @return std::unique_ptr<IDataRegistry::IDataWriter> The return pointer is always valid but the
 * writer methods will return ERR_NOT_INITIALISED if the module is not at least in state FS_READY or
 * the outgoing data has been unregistered.
 */
[[deprecated(
    "fep3::addDataOut is deprecated. Please use fep3::base::addDataOut instead.")]] inline std::
    unique_ptr<arya::IDataRegistry::IDataWriter>
    addDataOut(arya::IDataRegistry& data_registry,
               const std::string& name,
               const arya::IStreamType& stream_type,
               size_t queue_capacity = 0)
{
    auto res = data_registry.registerDataOut(name, stream_type);
    return res ? data_registry.getWriter(name, queue_capacity) : nullptr;
}
