    ...
    for (const auto line: a_util::strings::split(lines, ",")) {
        ...
        if (name_value.size() != 2) {
            return CREATE_ERROR_DESCRIPTION(ERR_INVALID_ARG,
                                            "Line '%d' ('%s') doesn't contain a ':' separated key "
                                            "value pair 'original_name:renamed_name': '%s'",
                                            line_num,
                                            line.c_str(),
                                            lines.c_str());
        }
...
{
    FEP3_RETURN_IF_FAILED(configuration.registerPropertyVariable(
        _renaming_input, FEP3_SIGNAL_RENAMING_INPUT_CONFIGURATION_PROPERTY));
    FEP3_RETURN_IF_FAILED(configuration.registerPropertyVariable(
        _renaming_output, FEP3_SIGNAL_RENAMING_OUTPUT_CONFIGURATION_PROPERTY));
    return {};
}

Result DataSignalRenaming::unregisterPropertyVariables(base::Configuration& configuration)
{
    FEP3_RETURN_IF_FAILED(configuration.unregisterPropertyVariable(
        _renaming_input, FEP3_SIGNAL_RENAMING_INPUT_CONFIGURATION_PROPERTY));
    FEP3_RETURN_IF_FAILED(configuration.unregisterPropertyVariable(
        _renaming_output, FEP3_SIGNAL_RENAMING_OUTPUT_CONFIGURATION_PROPERTY));
    return {};
...
Result DataSignalRenaming::parseProperties()
{
    ...
    FEP3_RETURN_IF_FAILED(
        parse_string_to_map(static_cast<std::string>(_renaming_input), _map_renaming_input));
    FEP3_RETURN_IF_FAILED(
        parse_string_to_map(static_cast<std::string>(_renaming_output), _map_renaming_output));
    return {};
}
...
Result DataSignalRenaming::checkName(const std::string& name)
{
    if (!std::regex_match(name, std::regex("^[A-Za-z0-9_]+$"))) {
        RETURN_ERROR_DESCRIPTION(
            ERR_NOT_SUPPORTED,
            "Signal name '%s' is not supported. Use alphanumeric characters and underscore only!",
            name.c_str());
    }
    return {};
}