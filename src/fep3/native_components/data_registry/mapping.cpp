...
public:
    explicit RawMemoryPassPointerToListener(size_t emulated_data_size,
                                            ddl::mapping::rt::ISignalListener& signal_listener)
    ...
    {
        if (!_signal_listener.onSampleReceived(data, data_size)) {
            throw std::runtime_error(
                "Critical error: The simulation bus passed a nullpointer to the mapping engine!");
        }
...
public:
    ...
    {
        throw std::runtime_error(
            "Critical error: The simulation bus passed a stream type to the mapping engine!");
    }
...
a_util::result::Result SignalMapping::registerMappingConfiguration(
    const std::string& path_to_config_file, const ddl::dd::DataDefinition& ddl_description)
{
    a_util::xml::DOM dom;
    ...
    if (!path_to_config_file.empty()) {
        if (!dom.load(path_to_config_file)) {
            RETURN_ERROR_DESCRIPTION(
                ERR_INVALID_FILE,
                std::string("Failed to load mapping configuration file: " + dom.getLastError())
                    .c_str());
        }
    }
    else {
        RETURN_ERROR_DESCRIPTION(ERR_EMPTY,
                                 "Property '%s' is empty",
    ...
    if (!res) {
        RETURN_ERROR_DESCRIPTION(res.getErrorCode(),
                                 a_util::strings::join(_config.getErrorList(), "\n").c_str());
    }

    FEP3_RETURN_IF_FAILED(_engine.setConfiguration(_config));
...
a_util::result::Result SignalMapping::unregisterSignal(const std::string& target_signal_name)
{
    ...
    RETURN_ERROR_DESCRIPTION(ERR_NOT_FOUND, "Signal has not been registered");
}

a_util::result::Result SignalMapping::registerDataReceiver(
    std::shared_ptr<IDataRegistry::IDataReceiver> data_receiver,
    const std::string& target_signal_name)
{
    ...
    RETURN_ERROR_DESCRIPTION(
        ERR_NOT_FOUND,
        std::string("The signal " + target_signal_name + " is not registered").c_str());
}
    ...
    if (signal_handle_it != _mapped_signals.end()) {
        ...
        if (data_receiver_it == _data_receivers.end()) {
            RETURN_ERROR_DESCRIPTION(ERR_NOT_FOUND,
                                     "No data receiver is registered for this target signal");
        }
        else {
                                                     ...
                                                     handle_t& handle)
{
    ...
    if (!resolveType(type_name, type_description)) {
        RETURN_ERROR_DESCRIPTION(ERR_NOT_FOUND, "Source signal type not found in type description");
    }
    FEP3_RETURN_IF_FAILED(_data_registry.registerDataIn(
        source_name, base::StreamTypeDDL{type_name, type_description}));
    if (listener) {
        FEP3_RETURN_IF_FAILED(_data_registry.registerDataReceiveListener(
            source_name, std::make_shared<DataReceiverAdapter>(*listener)));
    }
    else {
...
a_util::result::Result SignalMapping::unregisterSource(handle_t handle)
{
    ...
    return _data_registry.unregisterDataIn(
        reinterpret_cast<DataRegistry::DataSignal*>(handle)->getName());
}

a_util::result::Result SignalMapping::sendTarget(handle_t target,
                                                 const void* data,
                                                 size_t size,
                                                 timestamp_t time_stamp)
{
    ...
    if (it_buffer != _sample_buffers.end()) {
        ...
        if (data_receiver_it != _data_receivers.end()) {
            ...
            RETURN_ERROR_DESCRIPTION(
                ERR_NOT_FOUND,
                "Unable to send target data: No data receiver registered at signal mapping");
        }
    }
    RETURN_ERROR_DESCRIPTION(ERR_NOT_FOUND, "No preallocated sample buffer found.");
}

a_util::result::Result SignalMapping::targetMapped(const char* target_name,
                                                   const char*,
                                                   handle_t target,
                                                   size_t target_size)
{
    _mapped_signals.emplace(std::make_pair(target_name, target));
    _sample_buffers.emplace(
        std::make_pair(target, std::make_shared<base::DataSample>(target_size, false)));
    return {};
}
...
fep3::Result SignalMapping::resolveType(const char* type_name, const char*& type_description)
{
    ...
    res ? type_description = description->c_str() : nullptr;
    return res;
}
...
a_util::result::Result SignalMapping::registerPeriodicTimer(timestamp_t,
                                                            ddl::mapping::rt::IPeriodicListener*)
{
    // This feature is not needed
    RETURN_ERROR_DESCRIPTION(ERR_NOT_IMPL,
                             "Periodic Timers to send mapped signals are not supported");
}

a_util::result::Result SignalMapping::unregisterPeriodicTimer(timestamp_t,
                                                              ddl::mapping::rt::IPeriodicListener*)
{
    RETURN_ERROR_DESCRIPTION(ERR_NOT_IMPL,
                             "Periodic Timers to send mapped signals are not supported");
