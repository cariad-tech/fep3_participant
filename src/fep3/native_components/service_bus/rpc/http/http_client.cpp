...
{
public:
    ::rpc::http::cJSONClientConnector& getConnector(const std::string& server_address, const std::string& service_name)
    {
...
void HttpClientConnector::setServerAddress(const std::string& server_address)
{
    ...
    // in case the host name server_address is the same as the participant, then we know the ip is 127.0.0.1
    ...
    IRPCRequester::IRPCResponse& response_callback) const
{
    try
    {
        ::rpc::http::cJSONClientConnector& con = _json_connector_cache->getConnector(_server_address, service_name);
