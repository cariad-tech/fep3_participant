...
private:
    ...
    public:
        ...
        Node(std::vector<std::string>& name, const LoggerFilterInternal& filter, const Node& parent);

        void setLoggerFilter(std::vector<std::string>& name, const LoggerFilterInternal& filter, bool overwrite = true);
...
public:
    ...
    *
    * This will also overwrite all already existing filters with a lower hierarchy level than this domain.
    * The overwrite behaviour can be disabled with the overwrite parameter.
    *
    ...
    */
    void setLoggerFilter(const std::string& logger_name, const LoggerFilterInternal& filter, bool overwrite = true);

    /**
    * @brief Returns a reference to the logging filter set for the logger name.
    *
    * If no specific filter has been set, it will return the filter of the next higher hierarchy level and so on.
    * If there is no filter for any level it will return the default filter of the configuration.
    *
