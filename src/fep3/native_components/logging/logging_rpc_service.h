...
public:
    explicit LoggingRPCService(LoggingService& logging_service) : _logging_service(logging_service)
    {
    }
...
private:
     ...
     * @param[in] logger_name       The logger name / domain to be configured. For more details see
     * @ref page_fep_logging_service
     * @param[in] severity          The filter level for this logger. All logs with a higher level
     * will be disregarded. To disable a logger use Severity::off.
     *
     ...
     */
    int setLoggerFilter(const std::string& enable_sinks,
    ...
    Json::Value getSinkProperty(const std::string& property_name,
                                const std::string& sink_name) override;
    int setSinkProperty(const std::string& property_name,
