...
Optional<std::string> get(const std::string& name)
{
    ...
    if (0 < length) {
        ...
        GetEnvironmentVariableW(
            wide_string_name.c_str(), &value_buffer[0], static_cast<DWORD>(value_buffer.size()));

        auto size_needed = WideCharToMultiByte(
            CP_UTF8, 0, &value_buffer[0], (int)value_buffer.size(), NULL, 0, NULL, NULL);
        std::string str_utf8(size_needed, 0);
        WideCharToMultiByte(CP_UTF8,
                            0,
                            &value_buffer[0],
                            (int)value_buffer.size(),
                            &str_utf8[0],
                            size_needed,
                            NULL,
                            NULL);
