...
public:
    FixedSizeRawMemory(size_t data_size) : _value(data_size, 0)
    {
    }
    ...
    size_t set(const void* data, size_t new_data_size) override
    {
        if (new_data_size == _value.size()) {
            ...
            _value.assign(static_cast<const uint8_t*>(data),
                          static_cast<const uint8_t*>(data) + new_data_size);
            return _value.size();
        }
        return 0;
    }
...
bool areEqual(const ::fep3::IDataSample& lhs, const ::fep3::IDataSample& rhs)
{
    ...
    return lhs.getTime() == rhs.getTime() && lhs.getSize() == rhs.getSize() &&
           lhs.getCounter() == rhs.getCounter() && lhs_read_bytes == rhs_read_bytes &&
...
bool haveEqualValue(const ::fep3::IDataSample& lhs, const ::fep3::IDataSample& rhs)
{
    ...
    FixedSizeRawMemory other_data_sample_raw_memory(rhs.getSize());
    auto rhs_read_bytes = rhs.read(other_data_sample_raw_memory);
    return lhs.getSize() == rhs.getSize() && lhs_read_bytes == rhs_read_bytes &&
           data_sample_raw_memory.value() == other_data_sample_raw_memory.value();
}

MATCHER_P(DataSampleSmartPtrValueMatcher,
          other,
          "Value equality matcher for smart pointer to IDataSample")
{
    const auto& arg_data_sample = *arg.get();
    const auto& other_data_sample = *other.get();
...
MATCHER_P(DataSampleSmartPtrTimestampAndValueMatcher,
          other,
          "Timestamp and value equality matcher for smart pointer to IDataSample")
{
    const auto& arg_data_sample = *arg.get();
    const auto& other_data_sample = *other.get();
