...
public:
    Implementation(size_t capacity) : _items(capacity <= 0 ? 1 : capacity)
    {
    }
...
DataItemQueue::~DataItemQueue() = default;

///@copydoc fep3::base::arya::detail::DataItemQueueBase::pushSample
void DataItemQueue::pushSample(const data_read_ptr<SAMPLE_TYPE>& sample,
                               fep3::arya::Timestamp time_of_receiving)
{
...
void DataItemQueue::pushType(const data_read_ptr<STREAM_TYPE>& type,
                             fep3::arya::Timestamp time_of_receiving)
{
    std::lock_guard<std::recursive_mutex> lock_guard(_impl->_recursive_mutex);
...
bool DataItemQueue::popFront(
    typename DataItemQueueBase<SAMPLE_TYPE, STREAM_TYPE>::IDataItemReceiver& receiver)
{
    std::lock_guard<std::recursive_mutex> lock_guard(_impl->_recursive_mutex);
...
///@copydoc fep3::base::arya::detail::DataItemQueueBase::popFront()
std::tuple<data_read_ptr<DataItemQueue::SAMPLE_TYPE>, data_read_ptr<DataItemQueue::STREAM_TYPE>>
DataItemQueue::popFront()
{
...
std::tuple<data_read_ptr<DataItemQueue::SAMPLE_TYPE>, data_read_ptr<DataItemQueue::STREAM_TYPE>>
DataItemQueue::read(size_t index) const
{
    data_read_ptr<SAMPLE_TYPE> sample = nullptr;
    data_read_ptr<STREAM_TYPE> stream_type = nullptr;
...
void DataItemQueue::reverseIteration(
    const std::function<bool(const data_read_ptr<SAMPLE_TYPE>& sample,
                             ...
                             fep3::arya::Timestamp)>& callback) const
{
    ...
    {
        if (!callback(item.getSample(), item.getStreamType(), item.getTime()))
            return;
    };
}

void DataItemQueue::iteration(
    const std::function<bool(const data_read_ptr<SAMPLE_TYPE>& sample,
                             const data_read_ptr<STREAM_TYPE>& stream_type,
                             fep3::arya::Timestamp)>& callback) const
{
