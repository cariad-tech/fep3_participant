...
{
    createWriter(qos_provider->datawriter_qos(topic->getSampleQosProfile()));

    _stream_type_writer = std::make_unique<DataWriter<fep3::ddstypes::StreamType>>(
        _publisher,
        topic->getStreamTypeTopic(),
        qos_provider->datawriter_qos(topic->getStreamTypeQosProfile()));
}
...
void StreamItemDataWriter::createWriter(const dds::pub::qos::DataWriterQos& qos)
{
    ...
    _sample_writer = std::make_unique<DataWriter<dds::core::BytesTopicType>>(
        _publisher, _topic->getSampleTopic(), qos, this, dds::core::status::StatusMask::none());
}
...
fep3::Result StreamItemDataWriter::write(const IStreamType& stream_type)
{
    try {
        ...
        for (const auto& property_name: stream_type.getPropertyNames()) {
            dds_stream_type.properties().push_back(
                fep3::ddstypes::Property(property_name,
        ...
        if (_topic->updateStreamType(stream_type)) {
            const auto qos =
                _topic->getQosProvider()->datawriter_qos(_topic->getSampleQosProfile());
            createWriter(qos);
        }
...
fep3::Result StreamItemDataWriter::transmit()
{
    try {
        ...
        /*
        The behavior of transmit() is not finally defined. Maybe we need to block until data was
        send _stream_type_writer->wait_for_acknowledgments(dds::core::Duration(0, 1000000));
...
void* StreamItemDataWriter::on_data_request(dds::pub::DataWriter<dds::core::BytesTopicType>&,
...
void StreamItemDataWriter::on_data_return(dds::pub::DataWriter<dds::core::BytesTopicType>&,
                                          void*,
                                          const rti::core::Cookie&)
{
