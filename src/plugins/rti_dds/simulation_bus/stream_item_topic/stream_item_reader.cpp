...
StreamItemDataReader::StreamItemDataReader(
    ...
    const std::weak_ptr<base::SimulationDataAccessCollection<ReaderItemQueue>>&
        data_access_collection,
    const std::shared_ptr<fep3::ILogger>& logger)
    : _topic(topic),
      _item_queue(std::make_shared<ReaderItemQueue>(_logger, topic)),
...
void StreamItemDataReader::reset(
    ...
    if (_data_access_iterator) {
        ...
        if (_logger->isDebugEnabled()) {
            _logger->logDebug(a_util::strings::format(
                "Replaced already registered data receiver for reader from topic '%s'.",
                _topic->GetTopic().c_str()));
        }
    }
