...
std::shared_ptr<fep3::IStreamType> createStreamType(const fep3::ddstypes::StreamType& dds_streamtype, const dds::sub::SampleInfo& /*sample_info*/)
{
...
std::shared_ptr<fep3::IDataSample> createSample(const dds::core::BytesTopicType& dds_sample, const dds::sub::SampleInfo& sample_info)
{
    ...
    , _subscriber(topic->getDomainParticipant(), topic->getQosProvider()->subscriber_qos(FEP3_QOS_PARTICIPANT))
{
    ...
    _streamtype_reader = std::make_unique<dds::sub::DataReader<fep3::ddstypes::StreamType>>
        ...
        , topic->getQosProvider()->datareader_qos(FEP3_QOS_STREAM_TYPE)
        );
}
            ...
            else
            {
                    ...
                    .take())
                {
                    ...
                    if (_topic->updateStreamType(*stream_type))
                    {
                        ...
                        // now recreate reader
                        const auto qos = _topic->getQosProvider()->datareader_qos(_topic->getQosProfile());
                        createReader(qos);
                    }
                }
            }
